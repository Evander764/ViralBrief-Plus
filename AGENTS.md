...
- 视频号巡检由 `server/rpa/wechat-desktop.js` 调用 `osascript` / System Events 操作桌面微信；用户必须先手动打开微信视频号窗口，停留在任意视频或视频号页面。RPA 从当前可见的视频号窗口开始：点击右上角小人入口 → 进入赞和收藏/个人总览 → 点击左侧栏“关注” → 保护标题含“关注”的总览标签 → 按账号池昵称打开博主主页。
- 默认不使用 Python、不使用 AI 视觉识别、不打开网页版视频号；不再从微信首页自动点击左侧视频号入口、不点击 Dock 微信图标、不重启微信；右侧下一个箭头使用固定窗口相对坐标，不做随机偏移；“关注”只允许在右上角小人入口后的左侧栏点击，禁止误点顶部视频流“关注”。
