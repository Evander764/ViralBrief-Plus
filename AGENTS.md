...
- 视频号巡检由 `server/rpa/wechat-desktop.js` 调用 `osascript` / System Events 操作桌面微信；用户需先手动打开一次微信视频号，让程序坞出现绿色的视频号独立窗口图标。RPA 从点击该程序坞图标开始：接管当前视频号窗口 → 点击右上角小人入口 → 进入赞和收藏/个人总览 → 点击左侧栏“关注” → 保护标题含“关注”的总览标签 → 按账号池昵称打开博主主页。
