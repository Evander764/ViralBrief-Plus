...
- 视频号巡检由 `server/rpa/wechat-desktop.js` 调用 macOS System Events / `screencapture` / Swift CoreGraphics 操作桌面微信，定位逻辑在 `server/rpa/wechat-locator.js`，重复 Swift 鼠标/像素工作统一走 `server/rpa/wechat-swift.js` 的预编译缓存助手；RPA 先激活桌面微信主窗口并确认主页面可截图/可读取 → 点击微信左上角搜索框 → 输入账号池昵称 → 进入第一个放大镜/网络搜索结果行 → 在搜一搜结果页点击“视频”标签 → 在视频结果主栏匹配目标视频号账号行 → 打开博主主页视频网格。
...
- 视频号窗口可以移动和缩放，所以左上搜索框、网络搜索结果行、搜一搜“视频”标签、目标视频号账号行、博主主页视频卡片和右侧下箭头都必须通过当前系统截图或可读 AX 控件现场识别后才允许点击。如果无法确认目标，只记录窗口锚点、候选坐标、截图特征和几何参考诊断并停止。
- 微信桌面巡检错误进入 API/UI 前必须清洗成短中文诊断；`details[].error` 和前端卡片不得透出 AppleScript/Swift 源码。若出现 `tell application`、`on vbp_`、`my vbp_` 等脚本片段，就是错误清洗回归。
- 每个博主默认读取 3 条，可通过 `rpa.wechatVideosPerAccount`（1-10）调整；主页视频按截图网格识别，置顶/直播/预约优先按 AX 暴露角标几何贴回卡片后跳过。
- 详情页切换下一条以在视频区域向下滚动为主路径，每次滚动后用截图指纹/AX 文案验证已经换片；滚动未换片时，再轻微晃动鼠标唤出右侧箭头，并用截图识别右侧下箭头点击兜底。点击“展开”只走 AX 按钮，展开后的文案只取 AX 可读文本，读不到完整文案时保存截图和诊断待补录，不调用 AI/OCR；采集点赞、转发、收藏/红心、评论。每个博主采满目标量或提前失败后发送两次 Command+W 回到微信主窗口/搜一搜标签页，未完成账号不得写 `last_patrolled_at`。
