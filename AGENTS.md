...
- 视频号巡检由 `server/rpa/wechat-desktop.js` 调用 `osascript` / System Events 操作桌面微信；RPA 先激活桌面微信主窗口并确认主页面可截图/可读取 → 从主页面左侧视频号入口进入并验证视频号界面；主页面入口失败时，才点击程序坞里的绿色视频号独立窗口图标兜底（优先匹配视频号/Channels/WeChatAppEx，多个“微信”时点击最右侧的那个）。进入视频号后继续：点击右上角小人入口 → 进入赞和收藏/个人总览 → 点击左侧栏“关注” → 保护标题含“关注”的总览标签 → 按账号池昵称打开博主主页。
- 默认不使用 Python、不使用 AI 视觉识别、不打开网页版视频号；不点击主微信 Dock 图标或相邻应用、不重启微信。右侧下一个箭头使用固定窗口相对坐标，不做随机偏移；“关注”只允许在右上角小人入口后的左侧栏点击，禁止误点顶部视频流“关注”。
- 进入关注总览后先关闭非“关注”标签，避免默认打开的视频继续播放。每个博主默认读取 3 条，可通过 `rpa.wechatVideosPerAccount`（1-10）调整；跳过置顶/直播/预约内容，打开第一条非置顶视频，点击“展开”读取完整文案，采集点赞、转发、收藏/红心、评论，每条保存截图证据后用右侧向下箭头切换下一条。
