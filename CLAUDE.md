...
- **视频号导航**：视频号入口是左侧竖排里「蝴蝶 / 横向 W 翅膀」形状的图标（视频号官方 logo），在相机光圈状的「朋友圈」图标**正下方**——别点成上方光圈图标（那是看一看/朋友圈，推荐流长得很像，已踩过坑）。入口 → 右上小人/赞和收藏 → 关注总览；这些固定按钮使用窗口相对坐标点击，不走视觉模型定位，避免 Retina/窗口坐标偏移。到达关注总览后，先截图确认顶部同时有左侧旧「视频号」标签和当前「关注」标签，再用中键关闭左侧旧「视频号」标签，保留当前「关注」标签；随后必须直接逐个查找关注博主并进入主页，不返回微信首页、不重新打开视频号 → 跳置顶 → 开第一条读数入库 → 触控板式双指上滑翻页（失败时滚轮兜底）→ 默认每博主 `cfg.wechat.maxVideosPerCreator`(3) 条 → 关详情回关注总览。
