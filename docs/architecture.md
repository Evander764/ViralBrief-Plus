...
- `server/rpa/wechat-desktop.js`：桌面微信视频号巡检入口，负责把底层 AppleScript/osascript 错误清洗为用户可读诊断。
...
前端网页巡检分两次调用：先 `/api/patrol/run` 跑 `platform = xiaohongshu`，阶段完成后再跑 `platform = douyin`。微信视频号巡检单独调用 `/api/patrol/run` 跑 `platform = wechat_channels`，只操作桌面微信：先激活微信主窗口，再点击左上搜索框输入账号池昵称，进入第一个放大镜/网络搜索结果行，切到搜一搜“视频”标签，匹配目标视频号账号行后打开主页视频网格；不使用网页版视频号，也不点击主微信 Dock 图标或相邻应用。视频号导航和详情翻页按系统全屏截图、代码特征识别、AX 可读文本和 CGEvent 坐标点击执行，不调用 AI 视觉或 OCR；识别不到目标时停止并保留诊断。服务端 `runDailyReport()` 自带 RPA 时只按小红书/抖音阶段执行；`runWechatReport()` 默认只整理已确认微信内容，显式 `skipRpa:false` 时才可先跑桌面微信视频号阶段。每批默认 6 个账号标签，配置范围 1-10，内存保护可下调。多标签模式下每个账号标签有独立 CDP client，跑完立即关闭。

微信巡检错误有一条额外边界：后端必须先用 `friendlyWechatDesktopError()` / `sanitizeWechatErrorText()` 把底层 AppleScript、osascript 和脚本源码清洗成短中文原因，再写入 `details[].error`；前端 `shortPatrolErrorText()` 只是第二道防线。API 响应和页面提示只保留“窗口诊断 / 搜索候选 / 视频号候选 / 截图定位”等短摘要，不能出现 `tell application`、`on vbp_`、`my vbp_` 等脚本片段。
