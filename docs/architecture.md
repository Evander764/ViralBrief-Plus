...
正式日报只收 `douyin` 和 `xiaohongshu`，且必须满足：
...
平台必需指标：小红书要求点赞和收藏都超过阈值；抖音要求点赞、收藏和转发都超过阈值。`1000+` 视为超过阈值，纯 `1000` 不入选。未知指标保留为 `null`，不能当成 0。弱来源、截图/OCR、普通页面文本或桌面视觉结果在人工确认前只进待复核；稳定 DOM/结构化 RPA 证据、人工确认和授权来源才可自动确认。

## 巡检排序与阶段

前端/API 实际按平台阶段执行：小红书阶段整体先于抖音阶段。每个阶段内部按评级从高到低排序，同评级按 `created_at` 更早优先；缺少添加时间时随机打散。`sortPatrolAccounts()` 仍保留混合列表的同评级小红书优先规则。默认跳过北京时间当天已经巡检过的账号。

前端一键巡检分两次调用：先 `/api/patrol/run` 跑 `platform = xiaohongshu`，阶段完成后再跑 `platform = douyin`。服务端 pipeline 自带 RPA 时也按这两个平台阶段执行。每批默认 6 个账号标签，配置范围 1-10，内存保护可下调。多标签模式下每个账号标签有独立 CDP client，跑完立即关闭。
...
5. 若发布时间早于窗口起点或无法识别，仍保存本条并继续按候选上限检查；后续由候选池人工复核或日报筛选排除。

互动指标、标题匹配和发布时间是否达标不在 RPA 阶段决定。未达阈值、缺少发布时间、超出窗口或标题不匹配的已打开详情都会先进入内容库；正式日报在巡检结束后通过 `recomputeAll(windowStartISO)` 和 `getEligible(windowStartISO)` 只选出账号池内、已确认、平台必需指标达标且发布时间在窗口内的内容。已采集过的详情页也要携带本次识别到的 `publishTime`，用于更新“最后看到的发布时间”。

浏览器上下滑动控制 API 不属于当前版本；此处只记录详情页采集顺序和批量打开行为。

## 平台细节

小红书：主页卡片按行从左到右检查，每个账号最多检查 `maxCandidatesPerAccount` 条候选，置顶内容跳过；每次点击必须位于上一次点击右侧，同一候选或同一详情重复打开会停止当前账号。详情保存前必须读取标题、作者、封面、视频时长、正文、发布时间、点赞、收藏、评论。可采详情先保存；保存或跳过不可采详情后优先点左上角关闭按钮回主页。

抖音：有主页 URL 则打开，无 URL 时通过搜索找账号。详情页先暂停视频，再读取封面/首帧、标题、正文、发布时间、点赞、收藏、转发、评论。可采详情先保存；是否达标留到巡检结束后的筛选阶段。
