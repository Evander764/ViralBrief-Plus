...
小红书：主页卡片按行从左到右检查，每个账号最多检查 `maxCandidatesPerAccount` 条候选，置顶内容跳过；每次点击必须位于上一次点击右侧，同一候选或同一详情重复打开会停止当前账号。详情保存前必须读取标题、作者、封面、视频时长、正文、发布时间、点赞、收藏、评论，并优先从标题/正文下方的“编辑于”区域读取发布时间。可采详情先保存；若非置顶详情发布时间早于本次窗口，保存本条后停止该账号后续候选；保存或跳过不可采详情后优先点左上角关闭按钮回主页。
