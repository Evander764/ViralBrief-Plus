...
前端网页巡检分两次调用：先 `/api/patrol/run` 跑 `platform = xiaohongshu`，阶段完成后再跑 `platform = douyin`。微信视频号巡检单独调用 `/api/patrol/run` 跑 `platform = wechat_channels`，只操作桌面微信：主微信窗口入口优先，绿色视频号独立 Dock 图标兜底，不使用网页版视频号。视频号导航和详情翻页按系统全屏截图、代码特征识别、AX 可读文本和 CGEvent 坐标点击执行，不调用 AI 视觉或 OCR；识别不到目标时停止并保留诊断。服务端 `runDailyReport()` 自带 RPA 时只按小红书/抖音阶段执行；`runWechatReport()` 可单独跑桌面微信视频号阶段。每批默认 6 个账号标签，配置范围 1-10，内存保护可下调。多标签模式下每个账号标签有独立 CDP client，跑完立即关闭。
