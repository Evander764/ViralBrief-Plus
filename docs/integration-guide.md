...
前端网页巡检会先调用一次 `platform = "xiaohongshu"`，完成后再调用一次 `platform = "douyin"`；微信视频号巡检单独调用 `platform = "wechat_channels"`，只操作 macOS 微信客户端：先激活微信主窗口，再点击左上搜索框输入账号池昵称，进入第一个放大镜/网络搜索结果行，切到搜一搜“视频”标签，匹配目标视频号账号行后打开主页视频网格；不使用网页版视频号，也不点击主微信 Dock 图标或相邻应用。服务端也仍支持 `platforms: ["xiaohongshu", "douyin"]` 的兼容写法。`maxTabsPerBatch` 默认 6，允许 1-10。
...
`details[].error` 是面向用户的短中文原因，不是底层调试日志。外部集成可以直接展示这段文案；不要依赖 AppleScript/osascript 原始文本。若响应里出现 `tell application`、`on vbp_`、`my vbp_` 等脚本片段，应按错误清洗回归处理。
