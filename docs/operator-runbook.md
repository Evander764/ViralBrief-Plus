...
微信视频号巡检只走独立按钮/API，不跟随网页日报混跑，也不打开网页版视频号。系统先激活桌面微信主窗口、确认主页面可截图/可读取，再从主页面左侧视频号入口进入；如果主页面入口不可用，才点击程序坞里的绿色视频号独立窗口图标兜底。两条路径都不满足时会失败并显示 Dock 标签、窗口名和关键文本诊断。接管视频号后会点击视频号窗口右上角小人入口；只有确认出现“赞和收藏/我的视频号/个人总览”入口后才继续。进入个人总览后，只点击左侧栏“关注”，不会点击顶部视频流“关注”。进入关注总览后会保护标题含“关注”的标签并关闭其他默认播放标签；后续点击都按“全屏截图 → 代码特征识别 → 坐标点击”执行，不调用 AI 截图识别或 OCR，重复 Swift 鼠标/像素工作会走预编译缓存助手以减少卡顿。找博主时先按账号池昵称收集 AX 候选并选择“以昵称为主体”的行，AX 不暴露列表文本或候选不可信时只在微信窗口内查找框粘贴目标昵称并用截图识别橙色高亮坐标，仍找不到才滚动关注列表并重新截图/读取；主页会用截图网格识别视频卡片，并把 AX 暴露的“置顶/直播/预约”角标贴回卡片后跳过。每个博主默认读取 3 条非置顶视频，可在设置里调整为 1-10 条；详情页只用 AX 点击“展开”和读取文案，读不到完整文案时保存截图待补录；翻页优先在视频区域向下滚动并验证换片，滚动未换片时才用截图识别右侧下箭头兜底。采集到的文案、每条截图、点赞、转发、收藏/红心和评论先进入候选池，人工确认后才进入微信日报。
