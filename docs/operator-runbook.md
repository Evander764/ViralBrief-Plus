...
微信视频号巡检只走独立按钮/API，不跟随网页日报混跑，也不打开网页版视频号。系统优先点击程序坞里的绿色视频号独立窗口图标；如果没有该图标，但当前普通微信窗口已经手动停在视频号界面，并且能通过页面文本验证为视频号，也会接管当前窗口。两者都不满足时会失败并显示 Dock 标签、窗口名和关键文本诊断。接管后会点击视频号窗口右上角小人入口；只有确认出现“赞和收藏/我的视频号/个人总览”入口后才继续。进入个人总览后，只点击左侧栏“关注”，不会点击顶部视频流“关注”。进入关注总览后会保护标题含“关注”的标签并关闭其他默认播放标签；每个博主默认读取 3 条非置顶视频，可在设置里调整为 1-10 条。采集到的文案、截图、点赞、转发、收藏/红心和评论先进入候选池，人工确认后才进入微信日报。
