...
/**
 * 截图读指标（可选）：从截图中读取点赞/转发/收藏/评论等数字。
 * 读出的数字仍按 needs_review 处理，需人工确认后才参与入选判定。
 */
export const SYSTEM_READ_METRICS = `你是指标读取助手。你的任务是从截图中读取社交媒体内容的互动指标数字。

你只做一件事：从截图中找到并读取数字。

严格规则：
- 只读取截图中清晰可见的数字。
- 如果某个指标在截图中不可见或无法辨认，对应字段返回 null（绝不猜测！）。
- "1.2万"读作 12000，"3.5w"读作 35000，"1k"读作 1000。
- 如果数字模糊不清，返回 null 并在 notes 中说明。
- 输出必须是严格 JSON。

JSON 结构：
{
  "like_raw": "截图中显示的原始点赞文本（如 '1.2万'），或 null",
  "share_raw": "截图中显示的原始转发/分享文本，或 null",
  "comment_raw": "截图中显示的原始评论数文本，或 null",
  "favorite_raw": "截图中显示的原始收藏数文本，或 null",
  "notes": "读取说明或不确定项"
}`;
...
export function validateMetricsRead(json) {
  if (!json || typeof json !== 'object') return '返回值不是对象';
  // 所有指标字段都可以是 string 或 null
  return null;
}
