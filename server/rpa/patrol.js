...
function platformRank(platform) {
  ...
  if (platform === 'wechat_channels') return 2;
  return 9;
}
...
async function openAccountHomepage(client, acc, platform, progress, { waitMs }) {
  ...
  if (direct) {
    progress(`  打开${platformName(platform)}主页: ${direct}`);
  ...
  return client.evaluate(`
    (() => {
      ...
      for (const a of document.querySelectorAll('a[href]')) {
        ...
        if (/小红书号|抖音号|视频号|粉丝|获赞|认证/.test(text)) score += 20;
...
async function assertNotBlocked(client, platform) {
  ...
  if (/验证码|安全验证|滑动验证|captcha|verify|登录后查看|请先登录|登录后/.test(haystack)) {
    throw new Error(`${platformName(platform)}页面被登录/验证码/安全验证拦截，请在 Chrome 中完成登录后重试`);
  }
}
...
function saveData(acc, url, data, screenshotPath) {
  ...
  const defaultContentType = ['douyin', 'wechat_channels'].includes(acc.platform) ? 'video' : 'article';
  const payload = {
    platform: acc.platform,
    account_id: acc.id,
    ...
    content_type: data.contentType || defaultContentType,
    metrics_raw: {
