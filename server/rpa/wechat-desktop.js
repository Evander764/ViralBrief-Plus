...
export async function runWechatDesktopPatrol({
  ...
  let prepared = null;
  try {
    prepared = await prepareWechatChannelsSearch({ scriptRunner, progress });
  } catch (e) {
...
async function patrolWechatDesktopAccount(acc, { index, total, progress, scriptRunner, now, shouldStop, opened, count }) {
  ...
  try {
    ...
    const openCreator = await runStep(scriptRunner, 'open_creator_by_main_search', { nickname: acc.nickname }, progress);
    ...
    await runStep(scriptRunner, 'close_creator_search_session', { keepTabTitle: '搜一搜' }, progress, { optional: true });
...
export async function prepareWechatChannelsSearch({ scriptRunner = defaultWechatScriptRunner, progress = () => {} } = {}) {
  const evidence = [];
...
export const prepareWechatChannelsFollowing = prepareWechatChannelsSearch;

export async function openWechatDesktopCreator(acc, { scriptRunner = defaultWechatScriptRunner, progress = () => {} } = {}) {
  const opened = await prepareWechatChannelsSearch({ scriptRunner, progress });
  const creator = await runStep(scriptRunner, 'open_creator_by_main_search', { nickname: acc.nickname }, progress);
...
function friendlyWechatDesktopError(e) {
  ...
  if (e?.code === 'main_search_field') {
    return appendWechatDiagnostics('没有找到微信左上角搜索框：请确认微信主窗口可见，左上角搜索框没有被遮挡', message);
  }
  if (e?.code === 'main_search_network') {
    return appendWechatDiagnostics('已经输入账号名，但没有点进“搜一搜”网络结果：请确认搜索下拉层里能看到第一个放大镜结果行', message);
  }
  if (e?.code === 'search_video_tab') {
    return appendWechatDiagnostics('已进入微信搜一搜，但没有切到“视频”标签：请确认搜索结果页顶部可见“视频”入口', message);
  }
  if (e?.code === 'search_video_account') {
    return appendWechatDiagnostics('已切到“视频”结果，但没有找到目标视频号账号行：请确认账号昵称能在视频结果里命中', message);
  }
  if (e?.code === 'creator_home') {
    return appendWechatDiagnostics('已点击目标视频号，但没有确认进入视频号主页：请确认页面已打开目标账号的视频网格', message);
  }
...
async function defaultWechatScriptRunner(step, payload = {}) {
  ...
  if (step === 'open_creator_by_main_search') {
    return await openWechatCreatorByMainSearch({ runner: execFileAsync, nickname: payload.nickname });
  }
  ...
  if (step === 'close_creator_search_session') {
    return await closeWechatSearchSession({ runner: execFileAsync });
  }
...
export async function openWechatCreatorByMainSearch({ runner = execFileAsync, nickname } = {}) {
  const targetNickname = String(nickname || '').trim();
  if (!targetNickname) {
    return { ok: false, code: 'open_creator_by_main_search', method: 'main_search', message: '缺少视频号博主昵称，无法搜索' };
  }
  try {
    const { stdout, stderr } = await runner('osascript', ['-e', appleScriptForMainSearchCreator(targetNickname)], { timeout: 35_000 });
    ...
    return {
      ...
      code: /not authorized|not allowed|assistive access|辅助功能|accessibility/i.test(String(e.stderr || e.message || ''))
        ? 'accessibility'
        : 'open_creator_by_main_search',
      method: 'main_search',
      message: String(e.stderr || e.stdout || e.message || '微信主搜索打开视频号失败').trim(),
    };
  }
}

async function closeWechatSearchSession({ runner = execFileAsync } = {}) {
  try {
    const { stdout, stderr } = await runner('osascript', ['-e', `
on run
  ...
  tell application "System Events"
    try
      ...
      try
        set value of attribute "AXMinimized" of window 1 of p to false
      end try
      try
      ...
      key code 13 using command down
      delay 0.45
      return my vbp_result(true, "close_creator_search_session", "command_w", "已尝试关闭视频号主页/搜一搜标签并回到微信主窗口")
    on error errMsg
      return my vbp_result(false, "close_creator_search_session", "command_w", errMsg)
    end try
  end tell
end run
...
`], { timeout: 8_000 });
    ...
    return {
      ok: false,
      code: 'close_creator_search_session',
      method: 'command_w',
      message: String(e.stderr || e.stdout || e.message || '关闭微信搜一搜标签失败').trim(),
    };
  }
}
...
function appleScriptForMainSearchCreator(nickname) {
  const targetNickname = appleString(nickname);
  return `
on run
  set targetNickname to ${targetNickname}
  tell application "System Events"
    if UI elements enabled is false then return my vbp_result(false, "accessibility", "ax", "辅助功能权限未开启")
  end tell
  tell application id "com.tencent.xinWeChat"
  ...
  tell application "System Events"
    ...
    if targetProcess is missing value then return my vbp_result(false, "not_logged_in", "main_search", "未找到桌面微信主进程")
    set visible of targetProcess to true
    set frontmost of targetProcess to true
    my vbp_raise_window(targetProcess)
    ...
    delay 0.08
    keystroke targetNickname
    delay 0.75

    if not my vbp_click_network_search_row(targetProcess, targetNickname) then
      return my vbp_result(false, "main_search_network", "main_search", "未点击到第一个放大镜搜索结果行；" & my vbp_search_diagnostics(targetProcess, targetNickname))
    end if
    delay 1.75
    set targetProcess to my vbp_process()
    if targetProcess is not missing value then my vbp_raise_window(targetProcess)

    if targetProcess is missing value or not my vbp_window_looks_souyi_sou(targetProcess) then
      return my vbp_result(false, "main_search_result", "main_search", "已搜索账号名，但未确认进入搜一搜结果页；" & my vbp_context_diagnostics(targetProcess))
    end if

    if not my vbp_click_search_video_tab(targetProcess) then
      return my vbp_result(false, "search_video_tab", "souyi_sou_video_tab", "未找到搜一搜顶部“视频”标签；" & my vbp_context_diagnostics(targetProcess))
    end if
    delay 1.25
    set targetProcess to my vbp_process()
    if targetProcess is not missing value then my vbp_raise_window(targetProcess)

    if targetProcess is missing value or not my vbp_click_target_video_account(targetProcess, targetNickname) then
      return my vbp_result(false, "search_video_account", "souyi_sou_video_account", "未在视频结果里找到目标视频号账号行；" & my vbp_video_account_diagnostics(targetProcess, targetNickname))
    end if
    delay 1.85
    set targetProcess to my vbp_process()
    if targetProcess is not missing value then my vbp_raise_window(targetProcess)

    if targetProcess is not missing value and my vbp_window_looks_creator_home(targetProcess, targetNickname) then
      return my vbp_result(true, "open_creator_by_main_search", "wechat_main_search", "已通过微信左上搜索框进入目标视频号主页：" & targetNickname)
    end if
    return my vbp_result(false, "creator_home", "wechat_main_search", "已点击视频号账号行，但未确认进入目标主页；" & my vbp_context_diagnostics(targetProcess))
  end tell
end run
...
on vbp_process()
  tell application "System Events"
    ...
    try
      return first process whose bundle identifier is "com.tencent.xinWeChat"
    end try
  end tell
  return missing value
end vbp_process
    ...
    try
      ...
      repeat with el in entire contents of targetProcessRef
        ...
        try
          set p to position of el
          set s to size of el
          set inTopLeft to ((item 1 of p) >= (item 1 of wp) + 90 and (item 1 of p) <= (item 1 of wp) + 540 and (item 2 of p) >= (item 2 of wp) + 20 and (item 2 of p) <= (item 2 of wp) + 110)
          if inTopLeft and ((roleText contains "TextField") or (roleText contains "text field") or (label contains "搜索")) then
            click el
            delay 0.18
            return true
          end if
      ...
      click at {(item 1 of wp) + 260, (item 2 of wp) + 58}
      delay 0.18
      return true
    end try
  end tell
  return false
end vbp_click_main_search_field

on vbp_click_network_search_row(targetProcessRef, targetNickname)
  tell application "System Events"
    try
      set w to window 1 of targetProcessRef
      set wp to position of w
      ...
      repeat with el in entire contents of targetProcessRef
        set label to my vbp_text(el)
        if label contains "搜索网络结果" then
          try
            set p to position of el
            set headerY to item 2 of p
            exit repeat
          end try
        end if
      end repeat
      repeat with el in entire contents of targetProcessRef
        set label to my vbp_text(el)
        if label contains targetNickname then
          try
            set p to position of el
            set s to size of el
            set yValue to item 2 of p
            set xValue to item 1 of p
            if xValue < (item 1 of wp) + ((item 1 of ws) * 0.72) and yValue > (item 2 of wp) + 145 then
              if headerY < 0 or (yValue > headerY and yValue < headerY + 170) then
                click at {(item 1 of wp) + 250, yValue + ((item 2 of s) / 2)}
                delay 0.25
                return true
              end if
            end if
      ...
      if headerY > 0 then
        click at {(item 1 of wp) + 250, headerY + 76}
        delay 0.25
        return true
      end if
    end try
  ...
  return false
end vbp_click_network_search_row

on vbp_window_looks_souyi_sou(targetProcessRef)
  try
    set dumpText to my vbp_visible_text_dump(targetProcessRef)
    if dumpText contains "AI搜索" then return true
    if dumpText contains "账号" and dumpText contains "朋友圈" and dumpText contains "视频" then return true
    if dumpText contains "搜一搜" and dumpText contains "相关搜索" then return true
  end try
  return false
end vbp_window_looks_souyi_sou

on vbp_click_search_video_tab(targetProcessRef)
  tell application "System Events"
    try
      set w to window 1 of targetProcessRef
      set wp to position of w
      set ws to size of w
      repeat with el in entire contents of targetProcessRef
        set label to my vbp_compact_label(my vbp_text(el))
        if label is "视频" then
          try
            set p to position of el
            set s to size of el
            if (item 2 of p) >= (item 2 of wp) + 150 and (item 2 of p) <= (item 2 of wp) + 370 and (item 1 of p) >= (item 1 of wp) + 240 and (item 1 of p) <= (item 1 of wp) + ((item 1 of ws) * 0.78) then
      ...
      click at {(item 1 of wp) + 620, (item 2 of wp) + 250}
      delay 0.25
      return true
    end try
  end tell
  return false
end vbp_click_search_video_tab

on vbp_click_target_video_account(targetProcessRef, targetNickname)
  tell application "System Events"
    try
      set w to window 1 of targetProcessRef
      set wp to position of w
      ...
      repeat with el in entire contents of targetProcessRef
        ...
        if label contains targetNickname then
          try
            ...
            set xValue to item 1 of p
            set yValue to item 2 of p
            if xValue < (item 1 of wp) + ((item 1 of ws) * 0.74) and yValue > (item 2 of wp) + 310 and yValue < (item 2 of wp) + ((item 2 of ws) * 0.88) then
              set scoreValue to 1000 - yValue
              if label contains "视频号" then set scoreValue to scoreValue + 500
              if label contains "条视频" then set scoreValue to scoreValue + 300
              if label contains "已关注" then set scoreValue to scoreValue + 80
              if scoreValue > bestScore then
                set bestScore to scoreValue
                set bestX to xValue + 170
                set bestY to yValue + ((item 2 of s) / 2)
              end if
            end if
          end try
        end if
  ...
  return false
end vbp_click_target_video_account

on vbp_window_looks_creator_home(targetProcessRef, targetNickname)
  try
    set dumpText to my vbp_visible_text_dump(targetProcessRef)
    if dumpText contains targetNickname and dumpText contains "直播回放" then return true
    if dumpText contains targetNickname and dumpText contains "音乐" and dumpText contains "视频" then return true
    if dumpText contains targetNickname and dumpText contains "商品橱窗" then return true
  end try
  return false
end vbp_window_looks_creator_home

on vbp_search_diagnostics(targetProcessRef, targetNickname)
  tell application "System Events"
    try
      set samples to {}
      repeat with el in entire contents of targetProcessRef
        set label to my vbp_text(el)
        if label contains targetNickname or label contains "搜索" or label contains "搜一搜" then
          if (count of samples) < 10 then set end of samples to my vbp_compact_text(label, 42)
        end if
      end repeat
      return "搜索候选=" & my vbp_join_list(samples, " / ") & "；" & my vbp_context_diagnostics(targetProcessRef)
    end try
  end tell
  return "搜索候选=无法读取"
end vbp_search_diagnostics

on vbp_video_account_diagnostics(targetProcessRef, targetNickname)
  tell application "System Events"
    try
      set samples to {}
      repeat with el in entire contents of targetProcessRef
        set label to my vbp_text(el)
        if label contains targetNickname or label contains "视频号" or label contains "条视频" then
          if (count of samples) < 10 then set end of samples to my vbp_compact_text(label, 48)
        end if
      end repeat
      return "视频号候选=" & my vbp_join_list(samples, " / ") & "；" & my vbp_context_diagnostics(targetProcessRef)
    end try
  end tell
  return "视频号候选=无法读取"
end vbp_video_account_diagnostics

on vbp_context_diagnostics(targetProcessRef)
  if targetProcessRef is missing value then return "窗口诊断=未找到微信进程"
  tell application "System Events"
    try
      set windowName to ""
      set contentCount to 0
      ...
      set dumpText to my vbp_visible_text_dump(targetProcessRef)
      return "窗口诊断=窗口 " & windowName & "，控件 " & (contentCount as text) & "，文本片段 " & my vbp_compact_text(dumpText, 260)
    end try
  end tell
  return "窗口诊断=无法读取"
end vbp_context_diagnostics
...
on vbp_visible_text_dump(targetProcessRef)
  if targetProcessRef is missing value then return ""
  tell application "System Events"
    set out to ""
    try
      repeat with el in entire contents of targetProcessRef
        ...
        if length of out > 7000 then exit repeat
      end repeat
    end try
    return out
  end tell
...
on vbp_compact_label(valueText)
  set s to my vbp_compact_text(valueText, 12)
  set s to my vbp_replace(s, " ", "")
  set s to my vbp_replace(s, tab, "")
  return s
end vbp_compact_label
