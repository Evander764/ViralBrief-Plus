...
1. **启动 RPA** → 自动拉起带有调试端口的独立 Chrome 实例；微信视频号优先直接点击程序坞里的绿色视频号独立窗口图标，只有该图标不存在时才需要用户先手动打开一次
2. **逐账号采集** → 小红书/抖音跳转主页并采集详情；视频号先点击程序坞里的视频号独立窗口图标，再从当前视频号窗口进入关注总览，按昵称打开博主并采集最新若干条视频
...
- **macOS 微信客户端** 已登录且允许辅助功能控制；巡检微信视频号前，程序坞里需要能看到绿色的视频号独立窗口图标，若没有该图标才先手动打开一次视频号
...
桌面微信 (视频号，优先直接点击程序坞独立窗口图标)
  ↓ osascript / System Events
点击视频号程序坞图标 → 接管当前视频号窗口 → 进入关注总览 → 按昵称打开博主 → 采集最新视频文案/指标/截图
  ↓
...
2. 程序坞里已有绿色的视频号独立窗口图标时，系统直接点击该图标继续；没有该图标时，用户才需要先手动打开一次微信视频号让它出现。系统不再从微信首页自动点击左侧视频号入口，不点击主微信 Dock 图标或相邻应用，也不重启微信。
3. 系统点击程序坞里的视频号独立窗口图标（优先匹配视频号/Channels/WeChatAppEx，多个“微信”时点击最右侧的那个），接管当前可见的视频号窗口，点击右上角小人入口，进入赞和收藏/个人总览。
4. “关注”只允许在右上角小人入口后的左侧栏点击；禁止把顶部视频流“关注”当成目标入口。进入关注总览后保护标题含“关注”的标签，关闭其他默认播放标签。
5. 按账号池昵称打开博主主页；每个博主默认读取 `rpa.wechatVideosPerAccount = 3` 条，可在 1-10 间调整。
6. 跳过置顶、直播和预约内容；打开第一条非置顶视频，暂停/展开文案，采集点赞、转发、收藏/红心、评论和截图，再用右侧向下箭头切到下一条。
7. 采集结果写入 `wechat-desktop://content/<account-id>/<date>/<index>`，指标证据写入 `metrics_evidence_json`；默认待复核，不自动确认，人工确认后才进入微信日报。
