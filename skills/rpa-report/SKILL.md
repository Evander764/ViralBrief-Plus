...
1. **启动 RPA** → 自动拉起带有调试端口的独立 Chrome 实例；微信视频号由用户先手动打开一次，让程序坞出现视频号独立窗口图标后再由独立入口接管
...
- **macOS 微信客户端** 已登录且允许辅助功能控制；巡检微信视频号前，需要先手动打开一次视频号，让程序坞出现绿色的视频号独立窗口图标
...
2. 用户必须先手动打开一次微信视频号，让程序坞出现绿色的视频号独立窗口图标；系统不再从微信首页自动点击左侧视频号入口，不点击主微信 Dock 图标或相邻应用，也不重启微信。
