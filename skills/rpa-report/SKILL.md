...
6. 按账号池昵称打开博主主页；先用 AX 文本匹配当前可见昵称，看不到就对关注列表执行 CGEvent 滚动，并在每次滚动后重新全屏截图/读取 AX；连续重复或到达底部仍找不到时停止，不走搜索或网页兜底。
7. 每个博主默认读取 `rpa.wechatVideosPerAccount = 3` 条，可在 1-10 间调整。主页视频卡片必须由系统全屏截图代码识别，跳过右下角有“箭头+横线”的置顶视频，以及 AX 可读的直播/预约内容；无法高置信识别第一条非置顶视频时停止，不点默认坐标。
8. 详情页先点击界面并轻微晃动鼠标唤出右侧箭头；“展开”只通过 AX 按钮点击，展开后的文案只取 AX 可读文本，读不到时保存截图和诊断待补录，不调用 AI/OCR；点赞、转发、收藏/红心、评论保持未知为 `null`，不把未知写成 0。
9. 每条保存截图证据后，用系统截图识别右侧下箭头并验证视频发生变化，再进入下一条。每个博主采满目标数或提前失败后发送两次 Command+W 回到“关注”总览；未完成账号不得写 `last_patrolled_at`。
