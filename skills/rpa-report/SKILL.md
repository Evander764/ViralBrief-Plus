...
全屏截图 → 代码特征识别 → 坐标点击 → 左侧栏视频号入口 → 右上角小人 → 左侧栏关注 → 悬停关闭关注左侧随机推荐标签 → 按昵称打开博主 → 采集最新视频文案/指标/截图
  ↓
...
6. 按账号池昵称打开博主主页；先收集 AX 中所有含昵称候选并选择“以昵称为主体”的行，AX 不暴露列表文本或候选不可信时只在微信窗口内查找框粘贴目标昵称并用截图识别橙色高亮坐标，仍找不到才对关注列表执行 CGEvent 滚动，并在每次滚动后重新全屏截图/读取 AX；连续重复或到达底部仍找不到时停止，不走网页兜底。
7. 每个博主默认读取 `rpa.wechatVideosPerAccount = 3` 条，可在 1-10 间调整。主页视频卡片必须由系统全屏截图代码识别，并把 AX 暴露的“置顶/直播/预约”角标按几何包含贴回卡片后跳过；无法高置信识别第一条非置顶视频时停止，不点默认坐标。
8. 详情页“展开”只通过 AX 按钮点击，展开后的文案只取 AX 可读文本，读不到时保存截图和诊断待补录，不调用 AI/OCR；点赞、转发、收藏/红心、评论保持未知为 `null`，不把未知写成 0。
9. 每条保存截图证据后，优先在视频区域向下滚动切到下一条，并用截图指纹/AX 文案验证视频发生变化；滚动未换片时才轻微晃动鼠标唤出右侧箭头，再用系统截图识别右侧下箭头兜底。每个博主采满目标数或提前失败后发送两次 Command+W 回到“关注”总览；未完成账号不得写 `last_patrolled_at`。
10. 采集结果写入 `wechat-desktop://content/<account-id>/<date>/<index>`，指标证据写入 `metrics_evidence_json`，包括截图定位来源、跳过置顶数量、展开状态、文案来源、下箭头点击点和 Command+W 收尾结果；默认待复核，不自动确认，人工确认后才进入微信日报。
