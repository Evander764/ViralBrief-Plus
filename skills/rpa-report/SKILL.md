...
1. **启动 RPA** → 自动拉起带有调试端口的独立 Chrome 实例；微信视频号由用户先手动打开视频号窗口后再由独立入口接管
2. **逐账号采集** → 小红书/抖音跳转主页并采集详情；视频号从当前可见的视频号窗口进入关注总览，按昵称打开博主并采集最新若干条视频
...
- **macOS 微信客户端** 已登录且允许辅助功能控制；巡检微信视频号前，需要先手动打开视频号窗口并停留在任意视频或视频号页面
...
接管当前视频号窗口 → 进入关注总览 → 按昵称打开博主 → 采集最新视频文案/指标/截图
  ↓
...
2. 用户必须先手动打开微信视频号窗口，停留在任意视频或视频号页面；系统不再从微信首页自动点击左侧视频号入口，不点击 Dock 微信图标，也不重启微信。
3. 系统接管当前可见的视频号窗口，点击右上角小人入口，进入赞和收藏/个人总览。
