...
test('账号池已添加账号和搜索填空都支持公众号与视频号平台', () => {
  const appJs = readFileSync(join(root, 'web', 'app.js'), 'utf8');
  const html = readFileSync(join(root, 'web', 'index.html'), 'utf8');

  assert.match(appJs, /const ACCOUNT_PLATFORM_OPTIONS = \['douyin', 'xiaohongshu', 'wechat_channels', 'wechat_article', 'other'\]/);
  assert.match(appJs, /const platformOpts = \(sel\) => ACCOUNT_PLATFORM_OPTIONS\.map/);
  assert.match(html, /id="acSearchPlatform"[\s\S]*value="wechat_channels">视频号[\s\S]*value="wechat_article">公众号/);
  assert.match(appJs, /\$\(\'#acSearchPlatform\'\)\.value = ACCOUNT_PLATFORM_OPTIONS\.includes\(item\.platform\) \? item\.platform : 'other';/);
  assert.doesNotMatch(appJs, /item\.platform === 'wechat_channels' \? 'other'/);
});
