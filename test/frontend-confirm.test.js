...
test('网页巡检前端保留两阶段与停止按钮；视频号桌面巡检使用桌面微信搜索路径', () => {
  ...
  assert.match(appJs, /function shortPatrolErrorText/);
  assert.match(appJs, /function wechatPatrolRecoveryHint/);
  ...
  assert.match(appJs, /视频号巡检\$\{statusLabel\}/);
  assert.match(appJs, /先让微信主窗口恢复到已登录首页/);
  assert.match(appJs, /tell application\|on vbp_/);
...
test('前端提供公众号文章批量导入入口', () => {
  const appJs = readFileSync(join(root, 'web', 'app.js'), 'utf8');
  const html = readFileSync(join(root, 'web', 'index.html'), 'utf8');

  assert.match(html, /id="wechatArticleUrls"/);
  assert.match(html, /id="wechatArticleImportBtn"/);
  assert.match(html, /id="wechatArticleImportResults"/);
  assert.match(html, /公众号文章导入/);
  assert.match(appJs, /function runWechatArticleImport/);
  assert.match(appJs, /parseWechatArticleUrls/);
  assert.match(appJs, /JSON\.stringify\(\{ urls \}\)/);
  assert.match(appJs, /accountMatched/);
});
