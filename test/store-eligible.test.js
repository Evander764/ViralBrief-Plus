...
test('getEligible 只返回账号池小红书/抖音里任一指标超过 1000 的 confirmed 内容', () => {
  ...
  const matched = upsertCapture({
    ...
    metrics_raw: { like: '10', share: '1500' },
  });
  confirmContent(matched.id, { like_count: 10, share_count: 1500, account_id: account.id });
  ...
  assert.equal(rows[0].eligible_reason, '转发/分享 1500');
});
